  ...
  initial: Array<
    {
      id: string;
      title: string;
      column: "backlog" | "ongoing" | "done";
      position: number;
      priority: "low" | "medium" | "high" | "urgent";
    }
  >,
) {
  const cards = initial.map((c) => ({
    ...c,
    ...
    handlers: (): InvokeHandlers => ({
      ...
      move_card: (args: Record<string, unknown>) => {
        ...
        if (c) {
          ...
          const max = Math.max(
            0,
            ...cards.filter((x) => x.column === a.column).map((x) =>
              x.position
            ),
          );
...
test.describe("Board drag-drop and keyboard move", () => {
  ...
  test("keyboard move via context menu: Shift+F10 -> ArrowDown -> Enter -> Move to Done", async ({ page, useTauri }) => {
    ...
    const done = page.locator('[data-column-id="done"]');
    await expect(done.locator('[data-testid="card-card-2"]')).toBeVisible({
      timeout: 5000,
    });
  });
});
