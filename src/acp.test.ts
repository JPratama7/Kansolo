...
// Pure logic tests for ACP GUI components.
...
function validateAgentForm(
  ...
  if (!command.trim() && !isBuiltIn) {
    return "Command cannot be empty for non-built-in agents";
  }
  return null;
}
...
Deno.test("AgentRegistry: validate form — empty name rejected", () => {
  assertEquals(
    validateAgentForm("", "claude-code", false),
