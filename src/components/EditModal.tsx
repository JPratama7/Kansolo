...
function EditModalForm(props: EditModalFormProps) {
  ...
  const [treeSourceId, setTreeSourceId] = createSignal(
  ...
  return (
    ...
    >
      <div
        class={`priority-strip ${PRIORITY_STRIP[props.card.priority]}`}
        aria-hidden="true"
      />
      <div class="p-4">
        <div class="flex items-start justify-between gap-3 mb-3">
          <Show
            when={!preview()}
            fallback={
              <h2 class="text-lg font-semibold text-ink leading-snug break-words">
                {title()}
              </h2>
            }
          >
        ...
        <Show when={!preview()}>
          <label
            class="block text-xs font-semibold text-ink-secondary mt-3 mb-1"
            for="edit-priority"
          >
            Priority
          </label>
...
export default function EditModal(props: EditModalProps) {
  ...
  const maxHeight = () =>
    columnRect() ? Math.min(columnRect()!.height, viewportH() - top() - 8) : 0;

  /** Close with a dirty check: first Esc shows a confirmation toast,
   * second Esc dismisses the toast, clean Esc closes directly. */
