...
export default function AgentRegistry() {
  ...
  const [activeByAgent, setActiveByAgent] = createSignal<
  ...
  async function refresh() {
    try {
      ...
      for (const r of active) {
        counts[r.agentName] = (counts[r.agentName] ?? 0) + 1;
      }
  ...
  async function save() {
    ...
    if (!n) return;
    // Only the built-in claude-code agent can have an empty command.
    if (!cmd && n !== "claude-code") {
      toaster.error({
        title: "Validation error",
        description: "Command cannot be empty for non-built-in agents",
      });
      return;
    }
    try {
  ...
  async function remove(name: string) {
    try {
      ...
      if (
        msg.includes("locked") || msg.includes("runs") || msg.includes("active")
      ) {
            ...
            onClick: () => {
              ...
              void (async () => {
                try {
                  ...
                  toaster.error({
                    title: "Delete failed",
                    description: acpErrorMessage(e2),
                  });
                }
              })();
            },
                  ...
                  <div class="flex items-center gap-2 shrink-0">
                    <Show when={agent.skills.length > 0}>
                      <span class="text-[10px] text-ink-secondary">
                        {agent.skills.length}{" "}
                        skill{agent.skills.length !== 1 ? "s" : ""}
                      </span>
                    </Show>
                    <span
                      class={`text-[10px] ${
                        agent.enabled ? "text-p-high" : "text-ink-secondary"
                      }`}
                    >
        ...
        <Show when={isFormOpen()}>
          <div class="flex flex-col gap-2 rounded border border-border-subtle p-3 bg-base/40">
            ...
            <Show when={skills().length > 0}>
              <div>
                <p class="text-xs font-semibold text-ink-secondary mb-1">
                  Skills
                </p>
