...
test('AgentRunPanel.pollUpdates: in-flight guard prevents double-append', async () => {
  ...
  render(() => <AgentRunPanel open={open()} onOpenChange={setOpen} run={run()} />);
  ...
  await waitFor(
    () => {
      if (invocations !== 1) throw new Error(`expected 1 invoke, got ${invocations}`);
    },
    { timeout: 4000 },
  );
  ...
  if (invocations !== 1) throw new Error(`guard failed: ${invocations} invocations`);
