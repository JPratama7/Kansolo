...
test("EditModal: dirty close shows confirmation toast, Discard closes modal", async () => {
  ...
  fireEvent.input(
    baseElement.querySelector("#edit-title") as HTMLInputElement,
    {
  ...
  if (!open()) {
    throw new Error("modal should stay open while dirty (open signal)");
  }

  fireEvent.click(getByText("Discard", { selector: "button" }));
  await waitFor(() => {
    if (open()) {
      throw new Error("modal should close after discard (open signal)");
    }
  });
