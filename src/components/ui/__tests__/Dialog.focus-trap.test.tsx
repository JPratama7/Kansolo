...
test("Dialog: Tab stays within Content; close restores focus to trigger", async () => {
  ...
  const trig = document.body.querySelector('[data-testid="trig"]') as
    | HTMLButtonElement
    | null;
  ...
  const content = document.body.querySelector('[data-testid="content"]') as
    | HTMLElement
    | null;
  ...
  if (focusables.length < 2) {
    throw new Error(`expected ≥2 focusable children, got ${focusables.length}`);
  }
  ...
  const trigAfter = document.body.querySelector('[data-testid="trig"]') as
    | HTMLButtonElement
    | null;
  ...
  if (document.activeElement !== trigAfter) {
    ...
    console.warn(
      "focus not restored to trigger under happy-dom (expected in real browser)",
    );
  }
