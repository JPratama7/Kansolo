...
test('App.handleSync: pauses on conflict, resumes after resolve with captured syncedAt', async () => {
  ...
  const syncResponses: Record<string, unknown> = {
    's1': {
      conflicts: [{
        sourceRef: 'JIRA-1',
        card: { id: 'c1', title: 'local', description: '', priority: 'medium', column: 'backlog', source: 'jira', position: 1, createdAt: '', updatedAt: '' },
        conflicts: [{ field: 'title', local: 'local', remote: 'remote' }],
        remote: { id: 'c1', title: 'remote', description: '', priority: 'medium', column: 'backlog', source: 'jira', position: 1, createdAt: '', updatedAt: '' },
      }],
      unmappedStatuses: [],
      syncedAt: 't1',
      importedCount: 1,
    },
    's2': { conflicts: [], unmappedStatuses: [], syncedAt: 't2', importedCount: 2 },
  };
  ...
  globalThis.window.__TAURI_INTERNALS__ = {
    invoke: async (cmd: string, args?: Record<string, unknown>) => {
      ...
      switch (cmd) {
        case 'list_sources':
          return [
            { id: 's1', sourceType: 'jira', label: 'Jira A', config: {}, statusMapping: { backlog: [], ongoing: [], done: [] }, enabled: true, createdAt: '' },
            { id: 's2', sourceType: 'jira', label: 'Jira B', config: {}, statusMapping: { backlog: [], ongoing: [], done: [] }, enabled: true, createdAt: '' },
          ];
        case 'sync_source':
  ...
  const syncArgs1 = calls['sync_source'].map((a) => (a as { sourceId: string }).sourceId);
  if (syncArgs1[0] !== 's1') throw new Error(`first sync should be s1, got ${syncArgs1[0]}`);
  if (calls['resolve_conflicts']) throw new Error('resolve should not run before apply-merge');
  ...
  fireEvent.click(baseElement.querySelector('[data-testid="apply-merge"]') as HTMLElement);
  ...
  await waitFor(() => {
    const syncArgs = calls['sync_source'].map((a) => (a as { sourceId: string }).sourceId);
    if (syncArgs.length !== 2 || syncArgs[1] !== 's2') {
      throw new Error(`loop did not resume to s2; syncs=${JSON.stringify(syncArgs)}`);
    }
    if (!calls['resolve_conflicts'] || calls['resolve_conflicts'].length !== 1) {
  ...
  await waitFor(() => {
    const setArgs = calls['set_setting']?.map((a) => (a as { key: string; value: string }));
    ...
    if (lastSynced.value !== 't2') {
      throw new Error(`expected last completed syncedAt 't2', got '${lastSynced.value}'`);
    }
  });
