...
const itemA = {
  runId: "r1",
  requestId: "r1:t1",
  description: "Read: foo.txt",
  timeoutMs: 60_000,
};
const itemB = {
  runId: "r2",
  requestId: "r2:t2",
  description: "Write: bar.txt",
  timeoutMs: 60_000,
};
...
test("PermissionDialog: two concurrent requests render one dialog", async () => {
  ...
  await waitFor(() => {
    ...
    if (!body.includes("Read: foo.txt")) {
      throw new Error("A description missing");
    }
  });
  const body = document.body.textContent ?? "";
  ...
  if (approveBtns.length !== 1) {
    throw new Error(`expected 1 Approve btn, got ${approveBtns.length}`);
  }
});
...
test("PermissionDialog queue: FIFO order", () => {
  enqueuePermission(itemA);
  enqueuePermission(itemB);
  if (permissionHeadSignal()?.requestId !== "r1:t1") {
    throw new Error("head should be A");
  }

  dequeuePermission("r1:t1");
  if (permissionHeadSignal()?.requestId !== "r2:t2") {
    throw new Error("head should advance to B");
  }
...
test("PermissionDialog: dequeue on respond", async () => {
  ...
  await waitFor(() => {
    if (permissionHeadSignal() !== null) {
      throw new Error("head not cleared after respond");
    }
  });
...
test("PermissionDialog: dequeue on timeout", async () => {
  ...
  try {
    ...
    if (permissionHeadSignal() !== null) {
      throw new Error("head not cleared on timeout");
    }
  } finally {
    vi.useRealTimers();
  }
