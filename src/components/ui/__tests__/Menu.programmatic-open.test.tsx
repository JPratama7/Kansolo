...
function mockInvoke(): void {
  ...
  globalThis.window.__TAURI_INTERNALS__ = {
    invoke: async (cmd: string, args?: { column?: string }) =>
      cmd === "list_cards_by_column" && args?.column === CARD.column
        ? [CARD]
        : [],
    transformCallback: () => 0,
  };
...
test("Menu: Shift+F10 opens card menu, ArrowDown moves focus, Escape closes", async () => {
  ...
  assertTrue(
    ...
    "a menu item is highlighted after ArrowDown",
  );
  ...
  assertTrue(
    ...
    `expected first or second menu item highlighted, got index ${
      Array.from(items).indexOf(highlighted[0])
    }`,
  );
