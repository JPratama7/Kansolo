...
const card: KanbanCard = {
  id: 'c1', title: 'Title', description: '', priority: 'medium',
...
const RECT = { left: 120, top: 80, width: 320, height: 600, right: 440, bottom: 680, x: 120, y: 80, toJSON: () => ({}) } as DOMRect;
const ZERO = { ...RECT, left: 0, top: 0, width: 0, height: 0, right: 0, bottom: 0, x: 0, y: 0 } as DOMRect;

function mockGBCR() {
  const orig = Element.prototype.getBoundingClientRect;
  Element.prototype.getBoundingClientRect = function () {
    return this.hasAttribute && this.hasAttribute('data-column-id') ? RECT : ZERO;
  };
...
function findWrapper(): HTMLElement {
  const content = document.body.querySelector('.ticket-panel') as HTMLElement | null;
  if (!content) throw new Error('Dialog.Content (.ticket-panel) not rendered in portal');
...
test('EditModal: anchored wrapper matches column rect left/top/width', async () => {
  ...
  render(() => (
    <EditModal card={card} treeSources={() => []} open={true}
      onOpenChange={() => {}} onSave={() => {}} />
  ));
  await new Promise((r) => setTimeout(r, 10)); // lazyMount + createEffect flush

  const w = findWrapper();
  if (w.style.left !== '120px') throw new Error(`left: expected 120px, got "${w.style.left}"`);
  if (w.style.top !== '80px') throw new Error(`top: expected 80px, got "${w.style.top}"`);
  if (w.style.width !== '320px') throw new Error(`width: expected 320px, got "${w.style.width}"`);
...
test('EditModal: centered fallback (inset:0 + flex classes) when no column element', async () => {
  ...
  if (!w.className.includes('flex') || !w.className.includes('justify-center')) {
