...
import { fireEvent, render, waitFor, within, cleanup } from '@solidjs/testing-library';
...
const cardA: KanbanCard = {
  id: 'a', title: 'Card A', description: '', priority: 'medium',
  column: 'backlog', source: 'local', position: 1,
  createdAt: '2024-01-01T00:00:00Z', updatedAt: '2024-01-01T00:00:00Z',
};
const cardB: KanbanCard = {
  id: 'b', title: 'Card B', description: '', priority: 'low',
  column: 'backlog', source: 'local', position: 2,
  createdAt: '2024-01-01T00:00:00Z', updatedAt: '2024-01-01T00:00:00Z',
};
...
test('EditModal race: switching cards while dirty prompts discard, no corruption', async () => {
  ...
  const queryDialog = () => baseElement.querySelector('[role="dialog"]') as HTMLElement | null;

  await waitFor(() => { if (!board().textContent?.includes('Card A')) throw new Error('cards not loaded'); });
  (within(cardArticle('Card A')).getByText('Edit') as HTMLButtonElement).click();
  await waitFor(() => { if (queryDialog() === null) throw new Error('modal did not open'); });

  const dialog = queryDialog() as HTMLElement;
  const editTab = dialog.querySelector('[data-value="edit"]') as HTMLElement;
  editTab.click();
  await waitFor(() => { if (!dialog.querySelector('#edit-title')) throw new Error('Title input not visible'); });
  fireEvent.input(dialog.querySelector('#edit-title') as HTMLInputElement, {
    target: { value: 'Card A edited' },
  });

  (within(cardArticle('Card B')).getByText('Edit') as HTMLButtonElement).click();
  await waitFor(() => {
  ...
  await waitFor(() => { if (queryDialog() === null) throw new Error('modal should remain on card B'); });
  if (!board().textContent?.includes('Card A')) throw new Error('card A title corrupted in board');
