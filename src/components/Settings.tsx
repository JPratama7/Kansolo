...
export default function Settings(props: SettingsProps) {
  ...
  const [EditComponent, setEditComponent] = createSignal<
    Component<SourceSettingsProps> | null
  >(null);
  ...
  // One active delete-confirmation toast at a time, so Esc/duplicates behave.
  ...
  function startEdit(src: SourceInstance) {
    ...
    if (!Comp) {
      setError(
        `No settings component registered for source type "${src.sourceType}".`,
      );
      return;
    }
  ...
  /** Confirm deletion via a persistent toast; dismiss any existing one first. */
  ...
  async function handleDeleteTree(id: string) {
    try {
      ...
      toaster.error({
        title: "Delete tree source failed",
        description: e instanceof Error ? e.message : String(e),
      });
    }
  }
      ...
      <Portal>
        ...
        <Dialog.Positioner class="fixed inset-0 z-50 flex items-start justify-center pt-10 px-4">
          ...
          >
            ...
            {error() && (
              <div
                class="px-4 py-2 bg-p-urgent/15 border-b border-p-urgent/40 text-p-urgent text-sm"
                role="alert"
              >
                {error()}
              </div>
                              ...
                              <div class="flex flex-col gap-3">
                                ...
                                <label class="flex items-center gap-2 text-sm text-ink">
                                  <input
                                    ...
                                    checked={editEnabled()}
                                    onChange={(e) =>
                                      setEditEnabled(e.currentTarget.checked)}
                                  />
                                  Enabled
                                </label>
                                <Show
                        ...
                        <fieldset class="border border-border-subtle rounded-[var(--radius-card)] p-3">
                          ...
                          <label class="flex items-center gap-2 text-sm text-ink">
                            <input
                              ...
                              checked={closeToTray()}
                              onChange={(e) =>
                                setCloseToTray(e.currentTarget.checked)}
                            />
                      ...
                      {id === "editor" && (
                        <fieldset class="border border-border-subtle rounded-[var(--radius-card)] p-3">
                          ...
                          <input
                            ...
                            value={editorCommand()}
                            onInput={(e) =>
                              setEditorCommand(e.currentTarget.value)}
                            placeholder="code…"
                          />
                          <p class="text-xs text-ink-secondary mt-1">
                            Default for the card right-click "Open in editor"
                            action. Each tree source can override this. Use{" "}
                            <code class="font-mono">{"{path}"}</code>{" "}
                            as a placeholder for the card's source path; if
                        ...
                        <fieldset class="border border-border-subtle rounded-[var(--radius-card)] p-3">
                          ...
                          <div class="flex flex-col gap-3">
                            ...
                            <div class="flex flex-wrap gap-2">
                              <label class="sr-only" for="new-tree-label">
                                Label
                              </label>
                              <input
                                id="new-tree-label"
                                ...
                                value={newTreeLabel()}
                                onInput={(e) =>
                                  setNewTreeLabel(e.currentTarget.value)}
                                placeholder="Label…"
                              />
                              <label class="sr-only" for="new-tree-path">
                                Path
                              </label>
                              <input
                                id="new-tree-path"
                                ...
                                value={newTreePath()}
                                onInput={(e) =>
                                  setNewTreePath(e.currentTarget.value)}
                                placeholder="path/to/source/folder…"
                              />
                              <label class="sr-only" for="new-tree-editor">
                                Editor command
                              </label>
                              <input
                                id="new-tree-editor"
                                ...
                                value={newTreeEditor()}
                                onInput={(e) =>
                                  setNewTreeEditor(e.currentTarget.value)}
                                placeholder="editor cmd (optional)…"
                              />
                              <button
                                type="button"
